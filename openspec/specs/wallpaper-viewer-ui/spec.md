...
MainWindow MUST 採用上下兩列版面: 上列為國旗 row, 下列為桌布圖片區. 國旗 row MUST 位於圖片區域**外圍** (上方), MUST NOT 疊加在圖片之上.

#### Scenario: 視窗一般大小

- **WHEN** 視窗大小為預設 (e.g. 1280x800)
- **THEN** 國旗 row MUST 出現在視窗頂部, 高度自適 (約 48-64 px)
- **AND** 圖片區 MUST 佔據國旗 row 之下的全部空間
- **AND** 國旗 row 與圖片區之間 MUST 無重疊

#### Scenario: 視窗縮小

- **WHEN** 使用者將視窗縮至最小 (e.g. 600x400)
- **THEN** 國旗 row 仍 MUST 完整可見且不壓圖
- **AND** 圖片區依比例縮小
...
應用程式啟動時 MUST 自動選中 us 國家並開始載入其今日桌布. 使用者不需做任何操作即可看到 (或正在載入) 美國今日桌布.

#### Scenario: 啟動

- **WHEN** 應用程式啟動完成 (主視窗已顯示)
- **THEN** 系統 MUST 自動觸發 us 桌布載入
- **AND** us 國旗按鈕 MUST 視覺上呈現 "已選中" 狀態 (e.g. 框線或反白)
